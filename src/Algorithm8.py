...
def azp(Xarr, Yarr, n_regions, w, max_iter=10000, min_size=None, init_stoc_step=True):
    ...
    while True:
        ...
        while len(region_list) > 0 and iters < max_iter:
            ...
            valid_moves = batch_check_AZP(moves, units, label, r, g, w, Xarr, Yarr, min_size)
...
def skater_reg(Xarr, Yarr, n_regions, w, min_size=None, verbose=0):
    nobs = Xarr.shape[0]
    nvar = Xarr.shape[1] - 1
    ...
    results = Skater_reg().fit(n_clusters=n_regions, W=w, data=Xreg,
              data_reg={'reg': spreg.OLS, 'y': Yreg, 'x': Xreg}, quorum=min_size, verbose=verbose)
